...
def mc_temp(prequestion_x, prequestion_xb, answera, answerb, answerc, answerd):
    # Initializing texts
    mc_temp.prequestion = Instruction_Box(
        m.transparent_box,
        1000,
        450,
        prequestion_x,
    ...
    mc_temp.prequestion_b = Instruction_Box(
        m.transparent_box,
        1000,
        450,
        prequestion_xb,
...
def multiplechoice_pq(game_number):
    ...
    if game_number == 4:
        mc_temp(
            m.prequestion_4,
            m.prequestion_4b,
            "a) 2 tegen de klok in, 4 tegen de klok in",
            "b) 4 met de klok mee, 2 tegen de klok in",
            "c) 2 met de klok mee, 4 met de klok mee",
            "d) 4 met de klok mee, 2 draait niet",
        )
        buttonwidth = 1050
    ...
    if game_number == 5:
        mc_temp(
            m.prequestion_5,
            m.prequestion_5b,
            "a) dat de muziek is opgenomen en wordt afgespeeld",
            "b) dat de muziek ter plekke wordt gespeeld",
            "",
            "",
        )
        buttonwidth = 1300
        ...
        answer_c_button = b.Button(
            m.transparent_box, m.transparent_box, m.transparent_box, 0, 0, 0, 0
        )
        answer_d_button = b.Button(
            m.transparent_box, m.transparent_box, m.transparent_box, 0, 0, 0, 0
        )
    ...
    while True:
        ...
        for event in m.pygame.event.get():
            ...
            if (
                ...
                and mc_temp.prequestion == mc_temp.prequestion_b
            ):
            ...
            if (
                event.type == m.pygame.MOUSEBUTTONDOWN
                and correct_button.mouse_on_button()
            ):
            ...
            if (
                answer_a_button != correct_button
                and event.type == m.pygame.MOUSEBUTTONDOWN
                and answer_a_button.mouse_on_button()
            ):
                m.wrong_answer_sound.play()
                mc_temp.answer_a = m.MagdaClean_font_50.render(
                    mc_temp.answera, True, m.red_color
                )
            # answer b) is wrong, it will turn red
            if (
                answer_b_button != correct_button
                and event.type == m.pygame.MOUSEBUTTONDOWN
                and answer_b_button.mouse_on_button()
            ):
                m.wrong_answer_sound.play()
                mc_temp.answer_b = m.MagdaClean_font_50.render(
                    mc_temp.answerb, True, m.red_color
                )
            # answer c) is wrong, it will turn red
            if (
                answer_c_button != correct_button
                and event.type == m.pygame.MOUSEBUTTONDOWN
                and answer_c_button.mouse_on_button()
            ):
                m.wrong_answer_sound.play()
                mc_temp.answer_c = m.MagdaClean_font_50.render(
                    mc_temp.answerc, True, m.red_color
                )
            # answer d) is wrong, it will turn red
            if (
                answer_d_button != correct_button
                and event.type == m.pygame.MOUSEBUTTONDOWN
                and answer_d_button.mouse_on_button()
            ):
                m.wrong_answer_sound.play()
                mc_temp.answer_d = m.MagdaClean_font_50.render(
                    mc_temp.answerd, True, m.red_color
                )
